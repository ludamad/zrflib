...
export function playGame(filePath:string) {
    ...
    let gameAi1 = makeGameAi(gameState, gameStateComp, evalBoard1, 4);
    let gameAi2 = makeGameAi(gameState, gameStateComp, evalBoard2, 4);
    
    for (var i = 0; i < 100; i++) {
        for (let gameAi of [gameAi1, gameAi2]) {
    ...
    return; // Addendum, hoisted helper functions below:
...
function printBoard(gameState:GameState) {
    var {width, height} = gameState.gameStateDesc.grids()[0];
    let boardString = '';
...
function makeGameAi(gameState:GameState, gameStateComp:GameStateComponents, evalBoard:()=>number, maxSearchDepth:number) {
    ...
    function findMove():CompoundMove {
        ...
        return aiObject.bestMove.clone();
    }
